...
public class AsyncApp42ServiceApi {
    ...
    public void findDocByQuery(final String dbName, final String collectionName,
                               final String valueBrand,
                               final String valueBrand_catno,
                               final App42StorageServiceListener callBack){
        final String keyBrand = "brand";
        final String keyBrand_catno = "brand_catno";
        Query q1 = QueryBuilder.build(keyBrand, valueBrand, QueryBuilder.Operator.EQUALS);
        Query q2 = QueryBuilder.build(keyBrand_catno, valueBrand_catno, QueryBuilder.Operator.EQUALS);
        final Query query = QueryBuilder.compoundOperator(q1, QueryBuilder.Operator.AND, q2);
        final Handler callerThreadHandler = new Handler();
        new Thread() {
            @Override
            public void run() {
